...
class FormPage(Resource):
    def render_GET(self,request):
        return '<html><body>this is mock</body></html>'
    ...
    def render_POST(self,request):
        ...
        print 'name=',name
        global i
        #p=request.content.read()
        #print p,':',len(p)
        #logging.debug('post:[%d][%d][%s]',i,len(p),time.ctime())
        print 'post:[',i,']'
        i=i+1
        result = '{"result":"ok","getString":"'+name+'","random":"'+ str(random.randint(1000,9999))+'"}'
        return result
        #return name
